...
def main():
    ...
    listbox_tasks = tk.Listbox(frame_tasks, width=67, height=20, selectbackground="red")
    ...
    entry_task.pack(pady=20,padx=40)

    button_add_task = tk.Button(root, text="Add Task", command=add_task)
    button_add_task.pack(side=tk.LEFT, padx=80, pady=10, ipadx=5, ipady=5)

    button_delete_task = tk.Button(root, text="Delete Task", command=delete_task)
    button_delete_task.pack(side=tk.LEFT, pady=10, ipadx=5, ipady=5)

    button_edit_task = tk.Button(root, text="Edit Task", command=edit_task)
    button_edit_task.pack(side=tk.LEFT, padx=80, pady=10, ipadx=5, ipady=5)
